...
GNSS の 1PPS を PIO で ns 分解能で捕捉し、そこから水晶の周波数誤差を推定して補正し、UTC と 1PPS 出力を保ち (PPS が切れているあいだも holdover で外挿する)、PPS を NMEA の UTC 秒に対応づけ、リアルタイムに可視化する。
この推定と補正は HAL 非依存の再利用可能な `no_std` crate に切り出してある。
...
- **`gnssdo`**: 移植可能な時刻同期のコア。水晶の周波数オフセット (ppb) を推定して holdover する `DisciplinedClock`、PPS エッジを分類する `PpsTracker`、出力位相の type-II servo `PhaseLockLoop` (P/I/D と Smith 予測子)、受信機の量子化誤差を引く `QErrCorrector` を持つ。no_std、整数のみ、依存ゼロ。
...
- 素の `cargo test` と `cargo build` は host で動くコア (`default-members = gnssdo, rp-pps, ntp-refclock, pico-10base-t`) だけを対象にする。単一テストは `cargo test -p gnssdo <name>`。
