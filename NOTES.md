...
- **精度限界の分析 (smart-friend + 実機)**: σ~300ns の支配律速を誤差二乗和で切り分け:
  ...
  - **試して効かなかった事**: ①周期を sigma-delta 小数 dither + I を milli-ppb 化 (8ppb 量子化除去) →
    リミットサイクル不変 = actuator 量子化が主因でない (但し分解能改善なので**採用維持**)。②deadband=0 で
    P 常時減衰 → ζ0.35 のまま振動残り出力ジッタ増。③Ki=1/512 で ζ0.71 → **遅延 d≈2 が ζ 公式を裏切り改善せず**。
  - **結論**: <300ns は **Smith 予測子で遅延 d を補償** or **freq/phase を単一ループ統合**、かつ**安定した信号**
    (窓際弱信号は PPS 19% 欠落 + キャプチャ毎に σ が 300〜1100ns とブレて経験チューニングが収束しない) が要る。
    holdover を詰めるなら **TCXO** ($1-2, 窓際の温度変動に効く) が最コスパ。**<15ns は GYSFFMANC+窓際の物理壁**
    (timing 受信機 u-blox LEA-M8T 等 + 良アンテナの別プロジェクト)。**sub-µs は実用十分=現実的な落とし所**。
