...
- **UTC 位相同期 stage ② (PIO ハード測定, 実装済)**: 位相を Instant でなく **PIO の生カウンタ差**で測る。
  ...
  - **将来 (smart-friend 助言)**: freq(EMA)と位相を別ループにしている今の構成は本質的に **1 つの type-II
    PI ループ** (位相→PI→周波数ステア、I 項=周波数推定) に畳むべき。2 ループが干渉し定常ドループを生む。
    ループ遅れ d を実測して PI を設計すれば k=1/16 より速く収束できる。
