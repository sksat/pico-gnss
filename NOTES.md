...
- **UTC 位相同期 stage ② (PIO ハード測定, 実装済)**: 位相を Instant でなく **PIO の生カウンタ差**で測る。
  ...
  mod 1秒なので K の整数秒誤差は無害)。この ns 測定でコントローラを回すと **±ms → ~300ns (デッドバンド)**
  に締まる。実測比較 (`webapp/plot_compare.py`): 測定が Instant σ~360µs → PIO 16ns (~2万×), 出力位相が
  旧 Instant 制御 ±1.2ms → 新 PIO 制御 ~300ns。`PPSGEN ... hwphase_ns=` を追加 (phase_ns と並べて比較可)。
  - 制御ゲインは **k=1/16** (k=1/4 だとループ遅れで ±15µs リミットサイクル)。残る擾乱は周回グリッチの取り
    こぼし → **捕捉プログラムに `jmp low` を足し、X=0 通過時の偽キャプチャを源で除去** (罠 14)。
