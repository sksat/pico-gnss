...
/// 慣習。立ち上がりエッジが制御対象で幅は時刻中立 (返り周期から引いて low 待ちにする)。
...
async fn naive_pps_out_task(mut out: Output<'static>) {
    ...
    loop {
        ...
        // パルス幅 (立ち下がりの時刻精度は不問: 制御対象は立ち上がりエッジ)。
...
async fn main(spawner: Spawner) {
    ...
    if use_pio(PRECISION_STAGE) {
        ...
        // SM1: GPSDO PPS 生成を GP3 へ (rp-pps の SteeredPpsOutput)。初期周期 = ppb=0 の 1Hz (内部で
        ...
        // モジュール/GPSDO の一般的な 1PPS 幅 (外部機器へ配れる; 制御対象は立ち上がりエッジで幅に不依存)。
