...
- **A. GPSDO ロック**: 推定した **RP2040 水晶の周波数誤差** (理想=GPS との差, ppb。+=水晶が速い) を時間軸で
  直接。起動で **+2.4ppm を学習しロック→平ら=保持**。定常 σ~数 ppb。(log の |距離| は 0 交差で下スパイク
  する artifact が出るので線形を採用。)
- **B. 時刻同期 ns 級**: GPSDO 補正後の UTC 予測残差。**MT3333 1PPS 仕様 ±10ns の帯 (緑) の内側**に
  収まる = ns 級の時刻同期。
- **C. 受信 PPS① と 自作規律 PPS② のジッタ**: ①GPS から受信した PPS入力、②自分で生成した規律PPS出力、
  の各パルスの「平均からのズレ」を散布図で (各点=1パルス)。**両方とも ±16ns (PIO 量子化) 以内** =
  受信と同じ綺麗さで生成できている (①は MT3333 仕様 ±10ns 相当)。
