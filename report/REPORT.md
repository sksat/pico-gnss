...
| 規律 PPS 出力 | **UTC 位相同期** | **σ ~35 ns**(良受信)/ ~113 ns(弱受信)、offset ~90 ns | 旧ソフトは ±1.4 ms (図D)。offset は boot や設定で ~1.3µs まで変動。PIO 測定 + Smith + 3 修正で達成 (後述) |
...
一方で σ（ジッタ）は独立で、offset が 1.3µs の瞬間でも σ は ~10ns だった。
...
*500ns/div、CH1（GPS）を中央に置いた。この回は offset が +1.25µs まで離れているが、σ は ~10ns（pp 32ns）と過去最良である。受信は良くジッタは小さいのに絶対 offset だけが大きい、σ（ジッタ）と絶対 offset の独立を示す実例（この boot は `PMTK386,0` を入れていた）。*
...
*良受信（9 衛星）で 22 分ロックさせた hwphase。隣接エッジ間のジッタは ~10ns と滑らかだが、周期 ~60〜100s で ±~300ns（全幅 ~1.2µs）ゆっくり wander する。このため σ は観測窓に依存し、隣接で ~10ns、~2 分窓で ~35〜46ns、~22 分で σ ~138ns になる。到達点や独立検証で挙げた σ35〜46ns は ~2 分窓の代表値で、長時間ではこの遅い wander が支配する。*
...
`PMTK386,0` はむしろ絶対 offset を悪化させた疑いがある（これを入れた長時間 boot で offset が ~1.3µs まで上がり、外した clean な boot では ~95ns に戻った。K はほぼ同じ）。厳密な A/B はしていないが、いずれも timing を詰める方向には働かないので両方とも不採用とした。
...
`qErr` を出し、ケーブル遅延補正と固定位置モードを持つタイミング受信機（例: u-blox ZED-F9T）に替えれば、sawtooth 補正が σ を下げ、ケーブル遅延補正が絶対 offset を下げ、固定位置モードが位置由来のジッタを下げる。
