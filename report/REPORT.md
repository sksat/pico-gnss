...
実機ログから外乱（系統ドリフトと測定フロア）を取り出し、任意ゲインのコントローラをオフラインで回す（リフラッシュ不要）。
...
結果は σ 300ns→35ns（~9×、mean~0、|max| 80ns）で、測定フロア RSS≈11ns に近づく。
...
このログの範囲（128s まで）では上昇への転じ（turnover）が見えないが、これは追従誤差を測っているためで、水晶のランダムウォーク床そのものに当たったかは外部基準なしには言えない。
...
時定数を伸ばすと出力は GPS PPS の短期ノイズを平均化して取り込まなくなるが、その分「出力 − GPS」の差には GPS 側のノイズがそのまま残り、σ は GPS ノイズ床へ向かって増える。
