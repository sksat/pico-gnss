...
| 規律 PPS 出力 | **UTC 位相同期** | **σ ~35 ns**(良受信)/ ~113 ns(弱受信)、offset ~90 ns | 旧ソフトは ±1.4 ms (図①)。offset は boot や設定で ~1.3µs まで変動。PIO 測定 + Smith + 3 修正で達成 (後述) |
...
|---|---|---|---|
| 受信良好 | 9 / 1.06 | ±~50ns で安定（lk=1） | σ ~20〜46ns、offset ~90ns |
| 受信不良 | 7〜8 / 1.3 前後 | ±~200ns を周期 ~60s でなめらかに往復（lk=1） | σ ~113ns、pp ~360ns（同じ揺れ） |
