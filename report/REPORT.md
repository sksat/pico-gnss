...
- **A. GPSDO ロック (log)**: `|freq − ロック値|` を log-y で。起動時 ~2400ppb の誤差から **~8s で学習・
  ロック**し、以後は定常ノイズフロア (σ~数 ppb) に張り付く = 水晶ドリフトを GPS で規律できている。
- **B. 時刻同期 ns 級**: GPSDO 補正後の UTC 予測残差。**MT3333 1PPS 仕様 ±10ns の帯 (緑) の内側**に
  収まる = ns 級の時刻同期。
- **C. PPS & 規律出力ジッタ**: 入力 PPS と規律 PPS 出力のジッタを各平均からのズレで重ねた。**どちらも
  PIO の 16ns 量子化が下限** (受信機 PPS は MT3333 仕様 ±10ns 相当)。
