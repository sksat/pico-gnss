...
| 規律 PPS 出力 | **UTC 位相同期** | **σ ~35 ns**(良受信、~2min 窓)/ ~113 ns(弱受信)、**絶対 offset 平均 ~77 ns**(イベント跨ぎ sub-100ns を観測) | 旧ソフトは ±1.4 ms (図D)。boot/設定で ~1.3µs と見えた offset はイベント時のペアリングずれが疑われ(後述)、現状 fresh boot + 生 tick 差ゲートで sub-100ns を観測。PIO 測定 + Smith + 規律改良で達成 |
