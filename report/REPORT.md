...
| 規律 PPS 出力 | **UTC 位相同期** | **σ ~35 ns**(良受信、~2min 窓)/ ~113 ns(弱受信)、offset ~90 ns | 旧ソフトは ±1.4 ms (図D)。offset は boot や設定で ~1.3µs まで変動。PIO 測定 + Smith + 3 修正で達成 (後述) |
...
この相対オフセットには「エッジ定義の違い」も含まれ、しかも支配的になりうる。
...
オシロの 1.65V とこの帯がズレ、かつ GP3（高速ロジック）と GPS PPS（受信機ドライブで鈍い）で立ち上がり勾配が違うと、その差がそのまま系統オフセットになる。
つまり ~90ns の相当部分は「出力が真にズレている」のではなく「何をエッジと定義してどこで測るか」の差であり、絶対値はこの定義に依存する。
like-for-like で比べるには、オシロのしきい値を RP2040 入力帯（0.8〜2.0V）に合わせて振り、その振れ幅を定義差の不確かさとして見るとよい（定義合わせ）。
