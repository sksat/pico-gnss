...
## 精度の限界と、これ以上の詰めどころ (smart-friend 分析 + 実機検証)
...
(type-II の固有振動, 周期 ≈ 2π√Ki ≈ 71 エッジ = 実測と一致, ζ≈0.35)。

→ **量子化 8ns 化や overclock は今やっても無意味** (測定は誤差の 0.1%)。詰めるべきは制御。だが実際に
sigma-delta 小数 dither (actuator 8ppb 量子化除去)・deadband=0・Ki=1/512 (ζ0.71) を試しても**改善せず**:
**ループ遅れ d≈2 が ζ 公式を裏切り**、かつ窓際の弱信号 (PPS 19% 欠落, キャプチャ毎に σ 300〜1100ns とブレる)
で経験チューニングが収束しない。**<300ns には Smith 予測子で遅延補償 + 安定した信号が要る** (本格作業)。

**コスパ順の詰めどころ**: ①d 実測 → Smith/ループ統合 (部品 0, 300→50-100ns 見込み) ②TCXO (holdover の
温度ドリフト, $1-2) ③測定 8ns 化は σ<50ns になってから ④**<15ns は GYSFFMANC+窓際の物理壁** (timing 受信機
+ 良アンテナの別プロジェクト)。**現実的な落とし所は sub-µs** (NTP stratum-1 には過剰品質)。
採用済の改善: **周波数を sigma-delta で sub-8ppb 化**(I 項が正確な周波数に整定でき分解能の壁を除去)。
