...
外部の NTP 同期済みホストと突き合わせた結果が以下である(probe 経由で測っているので、経路の遅延を時計の誤差と取り違える余地はない)。
...
`PMTK251` には ACK がなく、原理的にも作れない。
返事は片側がまだ採用していないレートで送ることになるからである。
仕様も切替に要する時間を規定していない。
確かめる以外に知る方法がないなら、確かめるしかない。
