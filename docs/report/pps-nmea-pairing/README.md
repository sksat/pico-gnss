...
測位周期ごとに、決まった顔ぶれが決まった順で出てくる。
この個体は毎秒 14 本を送る。
同じ UTC 秒を載せたものを 1 サイクルとみなして並べると、時刻を先頭フィールドに持つ 4 本は、どちらの baudrate でもこの順だった。

```text
GGA → … → RMC → ZDA → GST
```

サイクルの区切りをエッジで取ると、バーストが 1 秒を越える 9600 bps では並びが回転して見える。
UTC 秒でまとめるかぎり、順序は基準の取り方に依らない。

## RMC と ZDA

`rp-pps` は RMC で対応付けていた。
MT3333 の NMEA 仕様 §2.2.5 は RMC を **Recommended Minimum Specific GNSS Data** と展開する。
位置と進路と速度、そしてそれらがいつのものかという時刻を載せる、航法源が最低限出すべき一本である。
...
「そのパルスの直後に出て、いま起きたパルスの時刻を告げる」と読める。
対応付けに使うならこちらである。

ただし、バーストの中で先に出るという意味ではない。
仕様は送出順について何も書いていない。
実測では ZDA はサイクルの終わり近くにいて、RMC より 73 ms 遅れて届く (9600 bps)。
ZDA が持っているのは順番の優先ではなく、パルスとの対応の定義である。

ZDA に切り替えた。
誤りは残った。

## サイクルはパルスからどれだけ遅れて届くか

115200 bps では、時刻を持つ 4 本が 69 ms に収まる。
サイクルが 1 秒の中に丸ごと入るので、パルスとの関係がそのまま読める。
先頭の GGA が直前のパルスの 198 ms 後、ZDA が 263 ms 後で、仕様の「数百 ms 後に出る」に合う。

9600 bps では同じ 4 本が 765 ms に伸びる。
...
受信機が自分のパルスの 200 ms ほど後に喋り始めるという点が baudrate で変わらないなら、9600 bps のサイクルでも先頭は同じ位置にある。
そこから 650 ms 後の RMC はパルスの 850 ms すぎ、723 ms 後の ZDA は 950 ms すぎに来る。
実測では、直前のパルスから RMC が 913 ms、ZDA が 83 ms である。
ZDA の 83 ms は次のパルスを越えたあとの値で、越えなければ 1083 ms にあたる。

到着時刻でエッジを選ぶ方式では、この差がそのまま結果になる。
RMC は境界の 87 ms 手前にいて、バーストの揺れで両側に散る。
対応付けはコイン投げになる。
ZDA はほぼ必ず越えるので、規律 UTC は毎回 1 秒遅れる。
ZDA に替えて誤りが安定しただけだったのは、これである。

順番のうえで正しいセンテンスを選ぶことと、それが境界の手前に届くこととは別である。
...
RMC はこの窓では次のエッジの 55 ms 手前にいて、その 73 ms 後の ZDA は窓を越えるので、冒頭に写っているのは一つ前の周期のものである。
一組のセンテンスが境界をまたいで置かれている。
...
ZDA に替えても誤りが残ったのはこれである。
仕様のうえで正しいセンテンスを選んでも、届く時刻が振れていれば境界を跨ぐ。
...
ここまでの対応付けは、届いた時刻だけで決めている。
センテンスが来た瞬間にスロットへ入っているエッジが、その秒の持ち主になる。
9600 bps でこの決め方が破れたのは、時刻センテンスが境界の際に届いていたからである。

時刻ではなく順番で決める道がある。
エッジを待ち行列に積み、センテンスが来たら最も古いものから取り出す。
パルスとセンテンスが 1 対 1 で順番どおりに来るなら、到着が境界のどちら側でも正しい組になる。

置き換えなかったのは、弱点が入れ替わるだけだからである。
...
一度崩れると崩れたまま進むので、脱落の頻度を知らないうちは選べない。
115200 bps では時刻で決めても余裕が 700 ms 以上あるので、急いで替える理由も今はない。
