...
仕様は送出順について何も書いていない。
同じ UTC 秒を載せたセンテンスを一つのサイクルとしてまとめると、時刻を先頭フィールドに持つ 4 本は、どちらの baudrate でもこの順に届いた。
...
ログに残った到着時刻を、エッジからエッジまでの 1 秒に並べると以下のようになる。
...
時刻センテンスが届いてから次のエッジまで、どれだけ残っているかを 188 本ぶん測ると以下のようになる。

![次のエッジまでの余裕の分布](fig-margin.png)

0 ms と 1000 ms は、同じ境界を両側から見た値である。
残りが 0 に近いセンテンスは境界を跨ぐ寸前におり、1000 に近いセンテンスは跨いだ直後にいる。
RMC は 98%、ZDA は 68% が、境界から 100 ms 以内に落ちている。
