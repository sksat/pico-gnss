...
合ったが、直ってはいない。
秒境界のどちら側に落ちるかは変わらないまま、どちら側を正解と呼ぶかを入れ替えただけである。

## baudrate を 115200 bps に上げる

センテンスを替えても秒境界からは離れられない以上、バーストそのものを短くするしかない。
GPS-R には UART の baudrate を動かすコマンドがあり、MT3333 NMEA spec 2.3.14 の `PMTK251` がそれにあたる。
...
設定は firmware の再フラッシュを跨いで残り、既定に戻るのは full cold start か standby のときだけである (MT3333 NMEA spec 2.3.14)。
...
これは MT3333 NMEA spec の言うことをそのまま実装した形である。
GPZDA が「いま起きたパルスの時刻を、そのパルスの数百 ms 後に告げる」のなら、直前のパルスを取れば正しい。

9600 bps でこれが外れたのは、その前提のほうが崩れていたからだった。
GPZDA が届くのは自分のパルスの 1083 ms 後で、数百 ms ではない。
UART がバーストを運びきれず、受信機が仕様どおりに振る舞えなくなっていた。

前提を要らなくする道もある。
エッジを待ち行列に積んでおいて、センテンスが来たら最も古いものから取り出す。
到着時刻を見ないので、何 ms 遅れて届いても正しい組になる。
...
待ち行列が当てにしているのは、エッジとセンテンスが 1 対 1 で順番どおりに届くことである。
これは MT3333 NMEA spec のどこにも書かれていない。
仕様が保証しているのはパルスとの時間関係のほうで、順番のほうではない。
脱落の頻度も分からないので、仕様に裏打ちのない前提を、別の前提と取り替えることになる。

115200 bps では GPZDA がパルスの 263 ms 後に届く。
仕様の言うとおりの姿に戻ったので、時刻で決める方式がそのまま正しく働く。
崩れた前提を作りで回避するより、前提が成り立つ状態に戻すほうが筋がよい。
