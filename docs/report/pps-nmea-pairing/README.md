...
14 本のうち 6 本は GSV で、これは見えている衛星を並べていくセンテンスである。
1 本に 4 個までしか載らないので、衛星が増えれば本数が増える。

![バーストの内訳と、GSV が増えたときの伸びしろ](fig-gsv-growth.svg)

いまは 22 個から 23 個が見えていて 6 本になっている。
衛星が 4 個増えるごとに 1 本、73 ms ぶん伸びる。
余白は 150 ms しかないので、あと 8 個増えれば 1 秒を使い切る。

バーストが 1 秒を端から端まで使うので、その開始が少し揺れるだけで、どのセンテンスも境界の反対側へ移る。

## センテンスを替えると、届く時刻は変わるか

仕様のうえで ZDA が正しいことと、ZDA が境界の手前に届くことは、別の話である。
そこで、どちらのセンテンスがいつ届くのかを測った。
...
次に、届いてから次のエッジまで何ミリ秒残っているかを測る。
以下ではこれを余裕と呼ぶ。

![余裕の測り方](fig-margin-def.svg)

跨ぐ直前なら余裕は 0 に近づき、跨いだ直後なら次のエッジまでの 1000 ms 近くになる。
同じ「境界の際」が、跨ぐ前と後で分布の両端に分かれて出る。

連続する 188 サイクルについて測ると以下のようになる。

![余裕の分布](fig-margin.png)
...
センテンスの選び方で余裕が作れない以上、バーストそのものを短くするしかない。
GPS-R には UART の baudrate を動かすコマンドがあり、仕様 §2.3.14 の `PMTK251` がそれにあたる。
これで 115200 bps に上げると 1 文字が 87 µs になり、同じ 816 文字を 71 ms で送り終わる。
占有率にすると 85% が 7% になる。
