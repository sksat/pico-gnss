...
![起動直後 16 取り込みの 1PPS](fig-pps.png)
...
右パネルの Pico client はこの run で +58.4 µs 離れている。
送信の定数 `TX_LAG_NS` が、時刻の基準をカウンタ側へ移す前の時計で測った値のままで、噛み合って
いないためである。これは精度ではなく、噛み合っていない定数の大きさである。
ここのタイムスタンプは、その定数を測り直すための計器にあたる。

## PTP そのものはまだ測っていない

`tiny-ptp` は 4 つのメッセージと、そこから offset と平均経路遅延を出すところまでで、host の
テスト 22 本が覆っている。firmware からはまだ呼んでいない。
**この回で実機の数字があるのは、上の経路 (12 ns) と 1PPS 出力 (6.2 ns) までである。**
2 枚の板を PTP で同期させて、その精度をオシロで測るのは次になる。
