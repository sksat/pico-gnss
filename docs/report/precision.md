...
自然ドリフトは小さすぎて temp-FF の効果を測れない (前節)。
そこで基板を手で加熱し、数秒で 0.7〜1.4℃ 上がる速い熱外乱を与えて、temp-FF の on/off を比べた。
ON は production (stage 5)、OFF は stage 4 (recal と ABS_OFFSET は両方 on、temp-FF だけ off) で、受信は sats 約 11 で安定していた。

まず ON の応答そのものが大きい。
速い加熱で hwphase が +5000/−6000 ns の双極性過渡を起こし、約 40s かけて ≤100ns 帯へ戻る。

![temp-FF on の速い加熱応答。温度急変で hwphase が ±µs 飛ぶ](precision-figs/fig7-tempff-heat.png)

on/off を加熱イベントごとに比べた (整定後、mispair を除外、ΔT で正規化)。
...
まず確かなことは、ON でも OFF でも、速い熱外乱では hwphase が数百 ns から数 µs/℃ の規模で過渡的に飛び、≤100ns を維持できないことである。
fast な熱周波数ステップを、このループ帯域は排除できない。

temp-FF が効くかどうかは、このデータでは確定できない。
peak/℃ と ∫|hwphase|/℃ の中央値は ON が OFF より約 30〜40% 小さく、過渡を緩和する傾向は見える。
しかし n が小さく (3 対 4)、加熱の熱量とレートがハンド加熱でばらつくため範囲が重複し、全ペア比較では ON が小さいのは半数強にとどまる。
...
残る ~500 ns は feedforward の過反応ではなく、有限帯域の制御ループが実周波数ステップに整定するまでの固有応答である。
速い外乱への整定が遅れること自体は制御として原理的なもので、これを 0 にするにはループ帯域を無限に広げるしかない。
帯域を広げれば速い外乱の整定は速くなるが、定常の wander (本機の支配項。type-II ループ共振が受信機ノイズで励起される) は悪化する。
本機は狭帯域で定常 ≤100ns を取る設計なので、速い外乱時の整定過渡はその設計選択の必然のコストであり、追って消すべき欠陥ではない。
firmware で直せたのは過反応 (実周波数変化の ~1700 倍という明確なバグ) の方で、それは steering clamp で ~8〜14 倍除いた。
整定過渡をさらに縮めたいなら、それは帯域 vs wander の trade-off を動かす設計判断になる (定常精度を犠牲にする方向)。
...
- temp-FF の効果量：高速温度外乱への応答は測った (前節) が、on/off の効果量は確定できていない。ハンド加熱で熱量とレートがばらつき n も小さいため、確定には on/off 交互で各 10 回以上、加熱レートを揃え、固定ヒータと水晶近傍センサで測る必要がある。
