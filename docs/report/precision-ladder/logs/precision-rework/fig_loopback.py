...
日本語版 (レポート用 fig-loopback.png) と英語版 (repo README 用 fig-loopback-en.png) を出す。"""
import os
...
JA = dict(
    cap_gps="エッジ捕捉\n(GPS-R 用)", cap_lb="エッジ捕捉\n(loopback 用)", gen="GPSDO PPS\n出力生成",
    out="GPSDO PPS 出力", loop="loopback: 出力を GP4 へ戻す",
    title="2 つのエッジ時刻の差 = loopback 位相 (出力が GPS-R からどれだけずれているか)",
    fname="fig-loopback.png",
)
EN = dict(
    cap_gps="edge capture\n(GPS-R)", cap_lb="edge capture\n(loopback)", gen="disciplined PPS\ngeneration",
    out="disciplined 1PPS out", loop="loopback: output returned to GP4",
    title="difference of the two edge timestamps = loopback phase (output vs GPS-R)",
    fname="fig-loopback-en.png",
)
...
def draw(L):
    ...
    for (bx0, by0, bw, lab) in ((5.2, 2.75, 1.45, L["cap_gps"]), (5.2, 1.45, 1.45, L["cap_lb"]), (6.83, 2.75, 1.12, L["gen"])):
    ...
    ax.text(9.75, 3.72, L["out"], ha="right", fontsize=10.5, color="#2a5db0")
    bx = 9.1
    ...
    ax.text(6.15, 0.28, L["loop"], ha="center", fontsize=10.5, color="#2a5db0")

    # 何が測れるか
    ax.text(5.0, 4.7, L["title"], ha="center", fontsize=11.5)

    fig.tight_layout()
    fig.savefig(os.path.join(OUT, L["fname"]), dpi=110); plt.close(fig)
    print(f"wrote {L['fname']}")
