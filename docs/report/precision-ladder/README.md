...
1PPS は 1 秒に 1 回パルスを出し、そのパルスの始まりが UTC の秒境界を指す。パルスが high と low のどちらなのかは受信機と基板で決まる。この GPS-R は基板上の 74HC04 を 1 段通って出てくるのでアクティブ Low で、パルスの始まりは**立ち下がり**である (取扱説明書に「1PPS 出力 : C-MOS ロジック (3.3V) レベル, パルス幅 :100mS (アクティブ Low)」とある)。当初この記録は立ち上がりを秒境界として書いていたが、それは誤りだった。外部の NTP client にこの時計を読ませたところ 100 ms 遅れていて、そこから発覚した。極性とエッジの関係は [pps-nmea-pairing の「基準にしていたエッジ」](../pps-nmea-pairing/README.md#基準にしていたエッジ) に図がある。以下の位相の測定はすべて 2 つの立ち上がりエッジどうしの相対量なので数値そのものは変わらないが、その基準エッジは秒境界より 100 ms 遅い。精度は GPS-R ごとに違い、MT3333 のデータシートでは PPS の時刻精度は typical ±10 ns とされている。Pico 側をどれだけ詰めても、最後はこの ±10 ns が天井になる。この記録では、GPS-R の 1PPS に対して 100 ns 以内で合い続ける出力を目標にする。
...
これは firmware 側で抑えられる不具合だ。温度予測と残差推定の和を、出力周期へ渡す直前で ±100 ppb で頭打ちにする**リミッタ**を入れる。リミッタが効くのは出力へ渡す値だけで、holdover の時刻外挿には頭打ち前の推定値をそのまま使う。

リミッタ版を焼き、加熱レートを約 1.1℃/10s に揃えて A/B を取る。診断ログでも、出力へ渡る値が過渡中に ±100 ppb へ張り付いていて、リミッタが効いていることを確認できる。loopback 位相の過渡 peak/℃ は、リミッタなしの中央値 1241 ns/℃、最悪値 7079 ns/℃ から、リミッタありの中央値 600 ns/℃、最悪値 764 ns/℃ へ落ちる。

![リミッタの加熱 A/B。loopback 位相の過渡 peak/℃ は最悪のステップで大きく縮む](precision-figs/fig10-clamp-ab.png)

中央値では約 2 倍、最悪値では約 9 倍の改善になる。効きは激しいステップほど明確で、中央値が約 2 倍にとどまるのは、リミッタなし側の中央値が穏やかなステップに引かれているためだ。

それぞれの最悪ステップを実時間の波形で並べると、違いは桁で見える。リミッタなしは ΔT 1.2℃ の加熱で +8640 ns から −4656 ns まで振れるのに対し、ありは ΔT 0.78℃ の最悪ステップでピーク 592 ns(764 ns/℃)で済み、約 35 秒で ±100 ns 帯へ戻る。

![それぞれの最悪加熱ステップの実時間波形。リミッタなしは ±数千 ns 振れ、ありは 600 ns 弱の短い往復で済む](precision-figs/fig10b-clamp-transient.png)
...
速い外乱への過反応はリミッタで止めた。残っているのは、長時間運転の節で「温度に寄っているらしい」までしか言えなかった、定常の遅いふらつきへの効き目だ。別の日の長時間ログどうしを比べる手もあるが、日が違えば受信も室温の動きも違うので、差がフィードフォワードのものか日のものかを分けられない。

そこで、同じ夜の中で条件だけを動かす。構成はスリップ修正済みの全部入りから定期校正だけを外して固定し(校正で入る位相の段が比較を汚さないようにする)、温度フィードフォワードの適用だけを 30 分ごとにあり/なしへ交互に切り替えて、約 7 時間 50 分連続で走らせる(この切り替えごとの 30 分のひとまとまりを、以下**区間**と呼ぶ)。交互に何度も繰り返すのは、室温のゆっくりした変化が特定の側にだけ当たる偶然を避けるためだ。記録は firmware の loopback 位相(毎秒)と、オシロでの実測(GPS-R PPS と出力エッジの差)の両方を取り、切り替え直後の 5 分は整定として統計から除く。序盤はオシロ側の取得が復旧作業で欠けているので、図と統計は、その穴のあとの連続した 12 区間(あり/なし各 6)で両方の計器をそろえる。

![温度フィードフォワードだけを 30 分ごとに切り替えた夜通し運転。上: loopback 位相と基板温度、中: オシロ実測、下: 区間ごとの σ。なしの区間だけ σ が跳ね上がり、明け方に温度が平坦になると差が消える](precision-figs/fig11-tempff-abab.png)
...
ここまでで、同じ GPS-R の 1PPS に対して中心を安定させられること、カウンタのズレのスリップの機構を突き止めてピンの上の中心を +9 ns 前後まで寄せられたこと、残った実ドリフトの出どころ (カウンタの一周のコスト差) を突き止めて直したこと、温度の速い外乱にはリミッタが効くこと、定常の遅いふらつきは温度が主因で、温度フィードフォワードがその大部分を吸収することまでは確認できた。残っている宿題もいくつかある。
