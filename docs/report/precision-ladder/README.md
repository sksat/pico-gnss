...
仮説が正しいなら、確かめられる予言が 2 つ出る。tick を奪うのは設定関数の最後の強制ジャンプなので、レジスタを 1 本ずつ同じ値で書くだけでは読みは下がらず、設定関数を呼んだときだけ 2 tick 下がるはずだ。もう 1 つ、偽の捕捉のぶん、FIFO に 1 個余分に積まれるはずだ。
...
![予言の検証。設定関数を呼ぶたびに読みが下がり (4 回呼ぶと −128 ns = −8 tick)、レジスタを 1 本だけ書いても下がらない](precision-figs/fig-kpoke-poke.png)

予言どおりに割れた。レジスタを 1 本だけ書いてもどれも下がらず、設定関数を呼んだときだけ、1 回につきちょうど 2 tick 下がった。偽の捕捉で積まれる余分な 1 個も、FIFO にそのまま出ていた。定期校正は GP2 へ向ける切り替えと GP4 へ戻す切り替えで 2 回起きるので、1 回の校正あたり −4 tick になる。
