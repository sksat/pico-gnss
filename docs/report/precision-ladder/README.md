...
![カウンタのズレの原因の切り分け。上: 3 本のカウンタ (GPS-R 用 c0 と観測用 c3 は GP2、loopback 用 c2 は GP4) と、各自が見るピンと校正での切替の有無。下: 差ごとに効きうる原因。c0 − c3 は同じ GP2 で切替もないので個体差しか効かず、c0 − c2 は 3 つ全部が混ざる](precision-figs/fig-kexp-setup.png)
