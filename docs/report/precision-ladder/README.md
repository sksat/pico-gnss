...
![まず 3 本目を足して見張る実験。上: 3 本のカウンタと各自が見るピン。c2 は校正のとき一瞬 GP2 へ切り替わる (これが「校正での切り替え」)。下: 見張った 2 つの差の時間変化 (模式図)。同じ GP2 を見て切り替わらない c0 − c3 は平坦で個体差なし、c0 − c2 は校正のたびに −4 tick の段。この段はピンには出ない](precision-figs/fig-kexp-setup.png)
