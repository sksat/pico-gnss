...
3 つの構成をピンの上のずれで並べると、ここまでの流れが 1 枚に収まる。校正なしでは出力が GPS-R からずれていき、定期校正で平坦になり、切替量子を除く修正で中心が +39 ns から +9 ns まで下がる。

![修正まで入れた結果 (オシロ実測)。左: 校正なしだと出力は GPS-R からずれていく。右: 定期校正で平坦になり、修正で中心が +39 ns から +9 ns へ下がる (分布の幅は遅いふらつき)](precision-figs/fig-fix-result.png)
