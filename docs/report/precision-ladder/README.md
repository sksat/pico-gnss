...
![まず 3 本目を足して見張る実験。上: 3 本のカウンタと各自が見るピン。c2 は校正のとき一瞬 GP2 へ切り替わる (これが「校正での切り替え」)。下: 見張った 2 つの差の 2.4 時間 (実測)。同じ GP2 を見て切り替わらない c0 − c3 は平坦 (±2 tick) で 2 本は離れない (カウンタごとの癖もなし)、c0 − c2 は校正のたびに −4 tick 下がり 2.4h で −228 tick。この段はピンには出ない](precision-figs/fig-kexp-setup.png)
...
仮説が正しいなら、確かめられる予言が 2 つ出る。tick を奪うのは設定関数の最後の強制ジャンプなので、レジスタを 1 本ずつ同じ値で書き直すだけでは段は出ず、設定関数を丸ごと呼んだときだけ 2 tick 下がるはずだ。もう 1 つ、偽の捕捉のぶん、FIFO に 1 個余分に積まれるはずだ。

確かめ方は第一歩と同じ 3 本目を使う。ただし今度は見張るのではなく、この設定関数を、ふだん切り替わらない 3 本目にわざと打ち込む (純観測なので何をしても制御に響かない)。設定関数を丸ごと呼ぶ場合と、レジスタを 1 本ずつ同じ値で書き直す場合とを打ち分けて、c0 − c3 に段が出るかを見る。

![予言の検証。設定関数を丸ごと呼ぶと 4 回で計 −128 ns の段が下がり、レジスタ単体の同値書き込みではどれも動かない](precision-figs/fig-kpoke-poke.png)

予言どおりに割れた。レジスタを 1 本ずつ書いてもどのレジスタでも段は出ず、設定関数を丸ごと呼んだときだけ、1 回につきちょうど 2 tick 下がった。偽の捕捉で積まれる余分な 1 個も、FIFO にそのまま出ていた。定期校正は GP2 へ向ける切り替えと GP4 へ戻す切り替えで 2 回起きるので、1 回の校正あたり −4 tick になる。
...
直し方は、実験がそのまま教えてくれる。切替のときに設定を丸ごと書き直すのをやめ、どのピンを見るかのレジスタフィールドだけを書き換える。

```text
sm.execctrl().modify(|w| w.set_jmp_pin(GP2));  // 見るピン (EXECCTRL の一部) だけ書く。exec_jmp は起きない
```

設定関数を通さずレジスタを直接書くだけなら段が出ないことは、上の実験で確認済みだ。1 時間の検証で、定期校正が測るズレは 23 回中 20 回が −1 tick になり、ぴったり −4 は消えた。ピンの上の中心も、修正直前の計測の +46 ns (中心そのものがゆっくりゆらぐので、窓の取り方で +33〜+46 ns を動く) から +9 ns へ動いた。移動量の約 37 ns は、取りこぼしていた 32 ns と、このゆらぎの誤差の範囲で一致する。ここまでの節で +33 ns と書いてきた中心は、この修正のあとは +9 ns 前後になる。
