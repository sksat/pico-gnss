...
犯人が校正の切り替えだと分かっても、それだけでは直せない。切り替えの何が tick を奪うのかが分からないからだ。カウンタ (ステートマシン) がどのピンを見てどう回るかは、いくつかの設定レジスタで決まる。ピンを GP4 から GP2 へ変えるのは、本来そのうち「どのピンを見るか」の 1 つを書き換えるだけでいい。ところが firmware は、PIO を操作するライブラリが用意した設定関数でこれをやっている。この関数は、ピンだけ変えたいときでも設定レジスタ一式 (クロック分周や FIFO など) を毎回まとめて書き直し、しかも最後に、走行中のプログラムを top へ強制的に跳ばせる。

```text
sm.set_config(cfg_gp2);   // 校正の切り替え = この 1 行 (c2 を GP2 へ向ける)

// set_config の中身 (ライブラリ側、簡略化)
fn set_config(cfg) {
...
exec_jmp が tick を奪う理由は、捕捉プログラムの中身にある。前に載せた擬似コードでは省いたが、実物は一度エッジを捕まえたら、同じパルスを二度捕らないように立ち下がりまで待つ。

```text
top:                         // 設定関数 (校正の切り替え) はここへ強制ジャンプさせる
loop {
    wait until pin == high;  // 立ち上がり待ち。このあいだ X は毎サイクル減る
    push X to FIFO;          // 捕捉。この数命令のあいだだけ X が止まる
    wait until pin == low;   // 次のエッジに備える。このあいだも X は減る
}
```

本物の立ち上がりは 1 秒に 1 回だから、捕捉で X が止まる数サイクルは全カウンタで同じで、差を取れば消える。ところが校正の切り替えがこのプログラムを top へ飛ばすのは、エッジ処理の直後、パルスがまだ high の最中だ。pin はもう high なので「立ち上がり待ち」を素通りして即「捕捉」に入り、同じパルスをもう一度積んでしまう (偽の捕捉)。この強制ジャンプと偽の捕捉のあいだ X が止まる 4 サイクル、1 tick は 2 サイクルだから、切り替え 1 回あたりちょうど 2 tick 取りこぼす。これが立てた仮説だ。

この仮説が正しければ、確かめられることが 2 つある。tick を奪うのが設定関数の最後の強制ジャンプなら、レジスタを 1 本ずつ同じ値で書くだけでは読みは下がらず、設定関数を呼んだときだけ下がるはずだ。もう 1 つ、偽の捕捉のぶん、FIFO に 1 個余分に積まれるはずだ。

確かめ方は第一歩と同じ 3 本目を使う。ただし今度は見張るのではなく、この設定関数を、ふだん切り替わらない 3 本目にわざと打ち込む (純観測なので何をしても制御に響かない)。設定関数を呼ぶ場合と、レジスタを 1 本だけ同じ値で書く場合とを打ち分けて、c0 − c3 が下がるかを見る。

![設定関数を呼ぶたびに読みが下がり (4 回呼ぶと −128 ns = −8 tick)、レジスタを 1 本だけ書いても下がらない](precision-figs/fig-kpoke-poke.png)

実際、レジスタを 1 本だけ書いてもどれも下がらず、設定関数を呼んだときだけ、1 回につきちょうど 2 tick 下がった。つまり犯人は、レジスタの書き直し (クロック分周も含めて) ではなく、設定関数が最後に打つ exec_jmp のほうだ。偽の捕捉で積まれる余分な 1 個も、FIFO にそのまま出ていた。定期校正は GP2 へ向ける切り替えと GP4 へ戻す切り替えで 2 回起きるので、1 回の校正あたり −4 tick になる。
...
直し方は、実験がそのまま教えてくれる。切替のときに設定関数を呼ぶのをやめ、どのピンを見るかのレジスタフィールドだけを書き換える。

```text
sm.execctrl().modify(|w| w.set_jmp_pin(GP2));  // 見るピン (EXECCTRL の一部) だけ書く。exec_jmp は起きない
```

設定関数を通さずレジスタを直接書くだけなら読みは下がらないことは、上の実験で確認済みだ。1 時間の検証で、定期校正が測るズレは 23 回中 20 回が −1 tick になり、ぴったり −4 は消えた。ピンの上の中心も、修正直前の計測の +46 ns (中心そのものがゆっくりゆらぐので、窓の取り方で +33〜+46 ns を動く) から +9 ns へ動いた。移動量の約 37 ns は、取りこぼしていた 32 ns と、このゆらぎの誤差の範囲で一致する。ここまでの節で +33 ns と書いてきた中心は、この修正のあとは +9 ns 前後になる。
...
3 つの構成を並べると、ここまでの流れが 1 枚に収まる。定期校正なしでは出力が GPS-R からずれていき、定期校正で平坦になり、切替量子を除く修正で中心が下がる。ここではピンの上のずれから内部の loopback 位相を引いて、両方に乗る遅いふらつきを消し、中心だけを見ている。

![修正まで入れた結果。ピンの上のずれから内部の loopback 位相を引き、両方に乗る遅いふらつきを消して中心を見る。左: 定期校正なしだと約 +4 ns/min でずれていく。右: 定期校正で平坦になり、切替量子を除く修正で中心が +40 ns から +10 ns へ下がる](precision-figs/fig-fix-result.png)

残った −1 tick/2.5 分 (約 6 ns/min) が、起動時校正だけの構成でピンが歩いて見えていた実ドリフトの本体だ。ピンで見えていた +3.8〜+5.3 ns/min とおおむね同じ大きさで、冒頭の食い違いは切替の見かけを差し引くと消える。定期校正はこの実ドリフトを吸収するために残す。出どころはまだ特定していない。同じ GPS-R ピンを見る 2 本のカウンタは 2.4 時間見ても離れなかったので、残る容疑は 3 つの候補のうちの波形依存になる。非同期な GPS-R エッジと同期した出力エッジとで、エッジを捕まえる位置の平均がゆっくり動く筋だ。
