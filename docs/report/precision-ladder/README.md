...
exec_jmp が tick を奪う理由は、捕捉プログラムの中身にある。大事なのは、X は勝手に進むのではなく、プログラムが `X = X - 1` を実行したときだけ 1 tick 進む点だ (これが 16 ns 刻みの正体)。前に載せた擬似コードでは省いたが、実物はこうなっている。

```text
top:                          // 設定関数はここへ強制ジャンプさせる
...
3 つの構成を並べると、ここまでの流れが 1 枚に収まる。定期校正なしでは出力が GPS-R からずれていき、定期校正で平坦になり、切り替えの取りこぼしを直す修正で中心が下がる。ここではピンの上のずれから内部の loopback 位相を引いて、両方に乗る遅いふらつきを消し、中心だけを見ている。

![修正まで入れた結果。ピンの上のずれから内部の loopback 位相を引き、両方に乗る遅いふらつきを消して中心を見る。左: 定期校正なしだと約 +4 ns/min でずれていく。右: 定期校正で平坦になり、切り替えの取りこぼしを直す修正で中心が +40 ns から +10 ns へ下がる](precision-figs/fig-fix-result.png)

残った −1 tick/2.5 分 (約 6 ns/min) が、定期校正なしだとピンが歩いて見えていた実ドリフトの本体だ。ピンで見えていた +3.8〜+5.3 ns/min とおおむね同じ大きさで、冒頭の食い違いは切替の見かけを差し引くと消える。だから、修正しても定期校正はやめられない。消えたのは校正の切り替えが取りこぼしていた 32 ns だけで、本物のドリフトはそのまま残るので、放置すればやはりピンが歩く (左図)。定期校正はこの実ドリフトを吸収するために続ける。出どころはまだ特定していない。同じ GPS-R ピンを見る 2 本のカウンタは 2.4 時間見ても離れなかったので、残る容疑は 3 つの候補のうちの波形依存になる。非同期な GPS-R エッジと同期した出力エッジとで、エッジを捕まえる位置の平均がゆっくり動く筋だ。
