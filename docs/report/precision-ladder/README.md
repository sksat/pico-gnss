...
![まず 3 本目を足して見張る実験。上: 3 本のカウンタと各自が見るピン。c2 は校正のとき一瞬 GP2 へ切り替わる (これが「校正での切り替え」)。下: 見張った 2 つの差の 2.4 時間 (実測)。同じ GP2 を見て切り替わらない c0 − c3 は平坦 (±2 tick) で個体差なし、c0 − c2 は校正のたびに −4 tick 下がり 2.4h で −228 tick。この段はピンには出ない](precision-figs/fig-kexp-setup.png)

この第一歩は、3 本目をただ足して見張るだけだ。狙いどおりに切り分けられた。同じ GP2 を見る c0 − c3 は、2.4 時間ずっと平坦で (傾き 0.000 ns/min、残るは量子化の ±1 tick だけ)、個体差は無罪になった。しかもスリップの本体 c0 − c2 の −4 tick は、オシロで見るピンの上には出ない (2.4 時間平坦、傾き −0.006 ns/min)。ピンに出ないのだから、波形依存のように本物のドリフトが溜まっているのではなく、校正の切り替えが内部の読みにだけ作る見かけだ。実際この −4 tick は毎回ほぼ一定で (56 回中 42 回がぴったり) 温度とも無相関、連続ドリフトではなく校正 1 回ごとの固定量だった。切り替えが作り、次の校正が測り直して自分で打ち消す。こうして犯人は校正の切り替えに絞れた。

犯人は分かったが、切り替えの何が tick を奪うのかはまだ分からない。そこで第二歩では、今度は見張るのではなく、切り替えと同じ操作を 3 本目にわざと打ち込む。切り替えに使っているのは PIO を操作するライブラリの設定関数で、ピン番号だけ変えたいときでも設定レジスタ一式を毎回書き直す。そこで 3 本目 (純観測なので何をしても制御に響かない) に、値を変えない書き込みを 1 分おきに打ち分けた。設定関数を丸ごと呼ぶ場合と、設定レジスタを 1 本ずつ同じ値で書き直す場合とで、c0 − c3 に段が出るかを見る。

結果ははっきり割れた。レジスタを 1 本ずつ同じ値で書いても、どのレジスタでも段は出ない。設定関数を丸ごと呼んだときだけ、1 回につきカウンタがちょうど 2 tick 遅れる。

![次に、切り替えと同じ操作を 3 本目にわざと打ち込む実験。設定関数を丸ごと呼ぶと 4 回で計 −128 ns の段が下がり、レジスタ単体の同値書き込みではどれも動かない](precision-figs/fig-kpoke-poke.png)

犯人は設定関数の最後の一手だった。この関数はレジスタを書き終えたあと、走行中のプログラムに「先頭へ跳べ」という命令を割り込ませる。なぜそれで tick が抜けるのかは、捕捉プログラムの中身を見ると分かる。前に載せた擬似コードでは省いたが、実物は一度エッジを捕まえたら、同じパルスを二度捕らないように立ち下がりまで待つ作りになっている。

```text
先頭:                          // 設定関数 (校正の切り替え) はここへ強制ジャンプさせる
loop {
    pin が high になるまで待つ;  // 立ち上がり待ち。このあいだ X は毎サイクル減る
    FIFO に X を積む;            // 捕捉。この数命令のあいだだけ X が止まる
    pin が low に戻るまで待つ;   // 次のエッジに備える。このあいだも X は減る
}
```

本物の立ち上がりは 1 秒に 1 回だから、捕捉で X が止まる数サイクルは全カウンタで同じで、差を取れば消える。ところが校正の切り替えは、パルスがまだ high の最中にこのプログラムを「先頭」へ飛ばす。pin はもう high なので「立ち上がり待ち」を素通りして即「捕捉」に入り、同じパルスをもう一度積んでしまう (偽の捕捉)。この強制ジャンプと偽の捕捉のあいだ X が止まる 4 サイクル、1 tick は 2 サイクルだからちょうど 2 tick を、切り替えたカウンタだけが取りこぼす。定期校正は GP2 へ向ける切り替えと GP4 へ戻す切り替えで 2 回起きるので、1 回の校正あたり −4 tick になる。偽の捕捉なら FIFO に 1 個余分に積まれるはずで、その予言もログでそのまま確認できた。
