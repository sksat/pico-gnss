...
パルスが high と low のどちらなのかは受信機と基板で決まる。この GPS-R はアクティブ Low で、idle の high から 100 ms だけ low に落ちる。モジュールの出す 1PPS が基板の 74HC04 を 1 段通って反転するためで、取扱説明書にも「1PPS 出力 : C-MOS ロジック (3.3V) レベル, パルス幅 :100mS (アクティブ Low)」とある。秒境界を指すのはパルスの始まり、つまり**立ち下がり**で、立ち上がりはその 100 ms 後に来るパルスの終わりでしかない。

![アクティブ High と アクティブ Low で、秒境界を指すエッジが入れ替わる](../pps-nmea-pairing/fig-polarity.svg)
