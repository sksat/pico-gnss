...
まず、内部の loopback 位相そのものに校正が要る。出力と GPS-R は別々の PIO カウンタで捕捉していて、2 つのカウンタは数え始めの瞬間が違うから、読みは常に一定量ずれている。この差をここでは実効 K と呼ぶ。大きさは 2 つを有効化するコードの間隔で決まり、この firmware では実測でおよそ 5500 tick (88 µs ぶん)、目標 100 ns の 1000 倍ある。
...
定期校正なしでは、出力は GPS-R に対して +131.5 ns に寄っていて、オシロの単発測定のうち ≤100 ns に入るのは 35% だけになる。ここまでの改良を全部入れた構成(以後、全部入り構成と呼ぶ)では、実効 K のスリップが抑えられ、GPS-R 相対の中心は安定する。一方で、定期校正をかけても、出力ピンと GPS-R PPS ピンの間には、常に一定のずれが残る。pad や基板の配線、オシロのプローブを信号が伝わるのに、それぞれ少しずつ時間がかかるためだ。ドリフトと違って時間で動かない固定のずれで、loopback からは見えない。オシロでピンの上を直接見ると、全部入り構成の中心は +33 ns あたりにいて、0 ではない。

![定期校正の前後 (オシロ実測)。定期校正でスリップが抑えられ中心が安定する。残る +33ns は配線やプローブの遅れによる固定のずれ](precision-figs/fig2-s4-beforeafter.png)

この安定は長時間でも保つ。全部入り構成を数時間走らせたあとにオシロで 30 発測っても、中心は +26 ns (σ 41 ns) にいた。

この固定のずれは、オシロの観測結果を制御に使えば消すことができる。ただ、それでは firmware の中で処理が完結しなくなるので、やらない。オシロ計測にはプローブの遅れの差などのバイアスも入り得るので、それを firmware に焼き込むと計測系の癖まで固定してしまう。オシロは、ピンの上で実際に揃っているかを外から確かめる検証に使い、出力のチューニングには使わない。残る +33 ns を本当に詰めるなら、オシロではなく pad や配線の遅れを独立に測る必要がある。

なお、以前見えた 793 ns の隠れたずれは定期校正で消えていて、いま残っている +33 ns はそれとは別物の、配線由来の固定のずれだ。
...
どの測定にも共通する注意点は、ロックに入りきる前と、落ち着いたあとを混ぜないことだ。収束途中のオーバーシュートの最中に取ると、全部入り構成でも +216 ns、≤100 ns 率 0% のような値が出るため、mean だけでなく σ と窓別統計を見て、受信条件も一緒に残す必要がある。loopback 位相は便利な内部観測だが、ピンの上の実測ではない。ピンの上の話をするときは GPS-R の PPS を基準にオシロで確かめ、配線由来の固定のずれと、動作中のドリフトを分けて見る。