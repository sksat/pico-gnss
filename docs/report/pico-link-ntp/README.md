...
**揺れは unicast のほうが 1.5 倍締まっている** (1.72 対 2.53 µs)。
往復は 2 つのタイムスタンプで送信側の変動を打ち消せるが、片道はそれができない。

平均が 16 µs 違うのは、2 つの経路がそれぞれ別の送信定数に乗っているためで、どちらの平均も
まだ意味を持たない。`WIRE_LAG_NS` が片付くまでは、比べられるのは揺れのほうだけである。
...
置き換えた定数は 40 秒の尺度ですらビルドの違いで 80 µs 動いていたので、そこは進んだ。
