...
![Pico から switch へ送り、有線と AP 経由の無線で同じ PC に届く](../ntp-stratum1/fig-two-paths.svg)

板 1 枚からギガビットスイッチへの、送信 1 方向である。
受け側は同じ LAN に置いた PC で、有線と無線の両方でその LAN に居る。
broadcast は LAN 全体に配られるので、switch から直接来る分と AP を経由して無線で来る分の
二つが、同じホストに入る。
...
PC で UDP を待ち受け、**パケットが申告した送信時刻**と、**カーネルが付けた受信時刻**
(`SO_TIMESTAMPNS`) を比べる。
同じ 1 つが二度届くので、どちらで受けたかは `IP_PKTINFO` で分ける。
...
**線に出るところまでは µs で決まった。**
秒境界から線の第 1 ビットまでは 236.19 µs、jitter は 6.87 µs だった (n=60、オシロ実測)。
申告する送信時刻をこれに合わせると、申告と実際の送出は中央値 **+1.2 µs** (n=401、−12.9 から
+15.1) で一致する。この時点で server 側は ±15 µs である。

**受け取った PC で読むと ms になった。**
broadcast は LAN 全体に配られるので、AP を経由した無線でも同じホストに届く。
読んでいる量は **「パケットが申告した送信時刻 − カーネルが付けた受信時刻」** で、片道の
所要時間も、PC の時計のずれも、この 1 つの値に入っていて分けられない。

600 秒ぶんの記録で、上の送信時刻の補正を入れる前 (firmware が秒境界そのものを申告していた
とき) のものである。
...
無線が 2 桁大きいが、有線だけを取り出しても ms である。
補正を入れたあとの記録では、有線が **+751.4 µs、jitter 180.7 µs** (n=130)、無線が
**−143.6 ms、jitter 91.9 ms** (n=110) になり、数分おいて取り直すと有線 +652.4 µs (n=66) と
まだ動いていた。以下「これまで」の値としてはこの補正後のものを使う。

**読めない理由は 3 つある。**

- broadcast の client は経路を測れない。RFC 5905 §3 は、先に client mode で数往復して伝搬遅延を
  校正し、そのあと受信だけに移ることを想定しているが、送信だけの配線ではその往復が張れない。
  latency の動かない部分がそのまま時刻に乗る (上の表の左列がそれを含んだ値である)
- jitter は校正でも引けない (右列)。無線の 90 ms は AP がビーコンに合わせて broadcast を溜める
  ためで、その位相は client にも server にも見えない
- 受け側のホストの時計そのものが決まっていない。公開 NTP サーバ 3 台と往復して推定すると 10 ms
  開き、往復が長いサーバほど大きく外れた

server 側は ±15 µs で決まっているのに、受け取った側で読める値は有線でも 0.75 ms、無線では
143 ms で、経路によって桁が変わる。**この構成では server 自身の µs は読めない。**
...
その 90 個の**相加平均**を **offset**、**標準偏差**を **jitter** と呼ぶ。
同期精度はこの 2 つの組で言う。片方だけでは足りない。

- offset は測り方の癖 (校正の誤り) で動く。動いたことに気づけないと、そのまま誤差になる
- jitter はその癖では動かない。校正が間違っていても、jitter の大きさは変わらない

前の構成の値も同じ 2 つの統計量だが、平均している量が「申告 − PC の受信」で違う。
そちらの offset には経路の **latency** (片道の所要時間) が入っていて、分けられない。
...
![構成を変えると桁がどう動くか](fig-progression.png)

配り方を **broadcast のままそろえて**比べたのが、上 3 行である。
PC で受けていたときは無線で −143.56 ms、有線でも +751.41 µs だったのが、Pico 2 枚の直結では
**+12.87 µs、jitter 2.53 µs** になった (8 分、90 取り込み)。
**同じ配り方のまま、無線と PC の時計が測定系から外れたぶんだけ縮んでいる。**

そのうえで、受信ができたので往復 (unicast) も張れる。
こちらは **+29.03 µs、jitter 1.72 µs** で、jitter は broadcast の 1.5 分の 1 になる (「broadcast と
unicast」)。

どの行も、その run の全サンプルの相加平均と標準偏差である。
横軸はその平均の大きさで、対数になっている。
ただし**平均している量が上下で違う**。上は PC が読んだ「申告した送信時刻 − 受信時刻」で、
片道の所要時間と PC の時計のずれが入っていて分けられない。
下はオシロが 1 回のトリガで読んだ「Pico client の秒 − Pico server の秒」で、どちらも入らない。
縮んだのは、その分けられない部分が測定系から外れたからである。
...
この取り込みの Pico server は −0.81 µs だが、同じ 140 回の平均は +0.29 µs、jitter は 1.24 µs である。
...
Pico server の jitter は 1.4 µs である。
...
- **offset**: run ごとの平均が −8.1 〜 +9.7 µs
- **jitter**: run 内の標準偏差が 0.6 〜 3.5 µs
...
|---|---|
| 40 秒 (16 取り込み) | jitter 4.4 ns |
| 8 分 (90 取り込み) | jitter 1.3 µs |
| 18 分 (2 つの run のあいだ) | offset そのものが 14.5 µs 動く |

基準をカウンタ側に置いて消えたのは、いちばん上の **jitter** である。
...
| 時刻の基準 | offset (GPS の秒に対して) | jitter | コードに書いた補正 |
|---|---|---|---|
...
**同じ長さで比べて、jitterが 1.24 µs から 4 ns になった。**
パルスの位置を「タスクが起きたとき」ではなく「数えた目盛り」で決めた結果である。
起動し直しても同じところに戻る (2 行目と 3 行目で平均の差が 35 ns)。

**ただしこれは 40 秒での話である。**
上の「時間の尺度で見え方が変わる」のとおり、8 分で見れば jitter は 1.3 µs、18 分では offset が
14.5 µs 動く。消えたのは取り込みごとの jitter で、分単位のうねりは残っている。
...
`fig-progression.png` だけは 2 つの回のデータを読む。これまでのぶんは前回の受信ログ
(`logs/20260819-ntp-wired/paths.log`)、今回のぶんは `logs/20260820-pico-link/pair-mode-*.csv`
で、どちらも生データから計算する。本文の数値も同じ計算から取っている。
