...
配り方を main と同じ broadcast にそろえて測ると、Pico client が Pico server の秒を再現した
精度は **+12.87 µs、1 回ごとの揺れ 2.53 µs** である (8 分、90 取り込み)。
main で同じ broadcast を PC で受けたときは、無線で −136.3 ms、有線でも −3816.5 µs だった。
...
main の値と今回の値は、同じ量ではない。
main のものは PC の時計と受信処理まで含んでいて分けられず、今回のものは受け取った板の秒を
