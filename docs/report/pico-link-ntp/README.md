...
PC で受けていたときは無線で −136.31 ms、有線でも −3.82 ms だったのが、Pico 2 枚の直結では
...
横軸は平均の大きさで、対数である。
これまでの 2 行と今回の 2 行は同じ量ではない。
これまでのものは PC の時計と受信処理まで含んでいて分けられず、今回のものは受け取った板の秒を
GPS の秒に対して直接読んだものである。縮んだのは、その分けられない部分が消えたからである。
