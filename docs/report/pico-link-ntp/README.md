...
`fig-broadcast-series.png` の 90 取り込みは、broadcast を受けて時計を進める client で取った。
いまの client は broadcast を数えるだけで使わず、要求と応答で時刻を得る。
手元の unicast の run はすべてそちらである。
つまり 2 つのモードの数字は**別のファームウェアのもの**で、差がモードから来たのか実装から来たのかを
分けられない。

同じビルドの両モードを、同じ長さで測るまでは何も言えない。
