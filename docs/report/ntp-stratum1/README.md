...
それを 10BASE-T の NTP として線に出したところ、受け取った client からはこの server が 100 ms 遅れて見えた。
firmware 自身の数字には、その 100 ms がどこにも出ていない。
捕捉した間隔はどれも正確に 1 秒で、周波数の推定も影響を受けないためである。

この文書は、配った時刻が合っているかをどうやって確かめたかの記録である。
出てきた食い違いは 3 つある。
1PPS の極性による 100 ms、申告した送信時刻と実際に線へ出た時刻との 236 µs、そして無線で届くまでの 120 ms である。

物理層に PHY チップはなく、Pico の GPIO 2 本と抵抗 3 本だけで線を駆動している。
