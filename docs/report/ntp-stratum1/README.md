...
時刻を正しく持っていることと、それを正しく配れていることは別である。

この GPSDO は 1PPS を PIO で捕捉して水晶の誤差を補正しており、内部では 1 秒がきっちり 1 秒だった。
それを NTP のパケットに載せて線に出したところ、受け取った client からはこの server が 100 ms 遅れて見えた。
firmware 自身の数字には、その 100 ms がどこにも出ていない。
捕捉した間隔はどれも正確に 1 秒で、周波数の推定も影響を受けないためである。
見つけたのは、線の向こうの client とオシロスコープだった。

GPSDO が抱えている時刻は、そのままでは他の機械から読めない。
機械が時刻を尋ねる方法は NTP で、NTP は Ethernet の上に載っている。
そこまで運んで初めて、この時刻は使えるものになる。

普通なら Ethernet には PHY チップと、パルストランスが要る。
この 2 つを省いているが、省ける理由は別々である。

**PHY を省けるのは、10BASE-T が遅いからである。**
1 bit が 100 ns、Manchester 符号の半 bit で 50 ns しかない。
RP2040 の PIO は 125 MHz で 1 命令 8 ns なので、この符号を CPU を介さずそのまま出せる。
100BASE-TX 以降はスクランブルも等化も要るので、こうはいかない。

**パルストランスを省けるのは、送信しかしていないからである。**
線を駆動する側にいるあいだは、共通モードも絶縁も相手側の問題になる。
スイッチの RJ45 にはパルストランスが入っているので、こちらの GPIO とスイッチの中身とのあいだに直流の経路はない。

受信を作るなら、そうはいかない。
届く信号は差動で、その共通モードはこちらの GND とは無関係に決まる。
単端の GPIO 入力では受けられないのでコンパレータが要り、そのコンパレータが見る共通モードは、パルストランスが無ければこちらで面倒を見ることになる。

この構成は kingyoPiyo の [Pico-10BASE-T](https://github.com/kingyoPiyo/Pico-10BASE-T) による。
`pico-10base-t` はそれを Rust へ移植したものである。

実際に繋いだのは、Pico の GPIO 2 本と抵抗 3 本だけである。

![10BASE-T の配線](fig-hardware.jpg)

*Cat5 の被覆を剥いた芯線に抵抗を直接半田付けし、そのままギガビットスイッチへ入れている。*

GP17 が TX+、GP16 が TX− で、それぞれに 33 Ω を直列に入れ、対の間に 1 kΩ を渡す。
3.3 V のロジック出力がこれで 10BASE-T の差動振幅に落ちる。
こちら側にパルストランスは入れていないので、絶縁も共通モードの除去もスイッチ側のものに任せている。

![送信側だけを作った配線](fig-wiring.svg)

作ってあるのは 10BASE-T の**送信側だけ**である。
規格そのものは送信対と受信対を別に持つ双方向のもので、受信対の芯線はケーブルの中にある。
今回はそれを繋いでおらず、復号する実装も持っていない。

**規格に準拠した PHY ではない。**
線の上を流れるのは 10BASE-T の線符号そのもので、市販のスイッチがリンクを張ってフレームを転送し、受け側のホストが普通のパケットとして受け取る。
そこまでは本物である。
一方で、こちら側に絶縁はなく、受信対を繋いでいないので CSMA/CD も成立せず、送信振幅も規格の下限を下回っているはずである (33 Ω の直列と 100 Ω の負荷から計算した話で、実測はしていない)。
受け側の squelch がそれより緩いので通っている、という状態にある。

以下で「10BASE-T」と書くのは、この意味である。

送信側しか無いせいで、確かめたいことが確かめられない。
NTP のパケットは先頭バイトに 3 bit の mode を持っており、mode がそのやりとりでの役割を表す (RFC 5905 §7.3)。
...
往復にかかった時間と、時計そのもののずれは、この 4 つから分離できる。

送信側しか作っていない構成が出せるのは **mode 5**、一方向の broadcast だけである。
載っている時刻は server の送信時刻ひとつなので、client は経路の遅延を測れず仮定するしかない。

そこで二通りに確かめた。
一つは受信経路を SWD から借りて本物の unicast を 1 往復させる方法、もう一つは broadcast を実際の 10BASE-T でホストに受けさせる方法である。
前半はこの最初の道をたどる。
冒頭に書いた 100 ms は、そこで出てきたものである。

## 受信経路を SWD から借りる

client の要求を firmware まで届ける道が要る。
作ったのは送信側だけなので、10BASE-T は通らない。
...
それは client が測る往復時間にそのまま入るので、測りたい ms がその中に埋もれる。
...
## server が UTC から 100 ms 遅れていた

ブリッジができたので、手元の NTP client からこの server を問い合わせた。
client が出すのは offset と往復時間の 2 つで、offset は server が申告した時刻と client 自身の時計との差である。

比べる相手は、client を動かしている PC の時計になる。
その時計も UTC とぴったりではないが、後の節で公開サーバと突き合わせるとおり、ずれているとしても数 ms である。
server のほうは Stratum-1 として UTC を配ると言っている。
だからこの offset が ms を大きく超えていれば、それは server 側の誤りになる。

![捕捉するエッジを変える前と後](fig-edge.png)

上段が当初の状態で、offset は **平均 −100.23 ms、標準偏差 0.55 ms** (n=299)。
負の符号は、server の申告が PC の時計より 100 ms 遅れていることを表す。
つまり server は UTC から 100 ms 遅れた時刻を配っていた。
...
ホスト側の時計でもない。その不確かさは数 ms で、100 ms には二桁足りない。

## 100 ms はどこから入りうるか

この時計は 2 つのものを貼り合わせてできている。
1PPS のエッジが「秒の境目はいつか」を決め、NMEA のセンテンスが「その秒は何秒か」を決める。
時刻の**位置**と**名前**を別々に受け取るので、どちらが間違えるかで症状が変わる。

名前のほうが間違えると、時計は秒の単位で飛ぶ。
NMEA が持ってくるのは整数秒で、取得したログ 378 本すべて小数部は `.000` だった。
エッジとセンテンスの対応付けを間違えた場合も、ずれるのは 1 秒の倍数である。

いま出ているのは 100 ms で、秒より細かい。
名前の側では作れない量である。

コードの途中で足される可能性もない。
エポックを立てるのは捕捉値と整数秒の素の代入で、サーボもクランプも挟まらない。
`gnssdo` に 100 ms の定数はあるが (`CORR_CLAMP_NS`)、これは出力 1PPS の位相補正に対する 1 エッジあたりの制限で、時刻の読み出しには掛からない。

残るのはエッジだけになる。
捕捉している瞬間が、真の秒境界から 100 ms 離れている。
...
立ち上がりは PPS の終わりなので、捕捉点はマークより 104 ms 遅い。

## PPS 幅を変えて確かめる

これだけでは状況証拠である。
決め手は、幅を変えて誤差が同じだけ動くかどうかである。
...
途中で一度、誤った結論を出しかけた。
反転を有効にしたまま duty を測ると 20.1% high と出て、PPS が high 側に見えたのである。
...
最初は起動のたびに決めていた。
1.5 秒ぶんサンプリングして短いほうの振れを探し、それがマークだとして捕捉エッジを選ぶ。
...
追いにくかったのは、資料がモジュール側の記述と基板側の記述とに分かれていたからである。
太陽誘電の GYSFFMANC データシートが書いているのはモジュール自身の端子で、`GIO_7 (1PPS) CMOS Digital I/O, 3D_Fix 時のみ出力(パルス幅：100msec)` とあるだけで極性には触れていない。
同じ表の `RESET` には「アクティブ LOW」と書いてあるので、この文書は極性一般を書かない文書なのではなく、1PPS についてだけ書いていないのである。
MediaTek のソフトウェア仕様は 1PPS を rising edge 基準で説明する。
...
## client から見た offset と往復

![client から見た offset と往復時間](fig-unicast.png)

往復は中央値 6.05 ms で、これは probe の経路である。
Ethernet は片道も通っていない。
server の処理は 70 から 95 µs、`stratum` は 1、`root dispersion` は 1.007 ms で、`origin timestamp` の echo は全交換で一致していた。
...
ここから先は broadcast を実際の線でホストに受けさせた記録である。
受け側は同じ LAN に置いた PC で、UDP を待ち受け、パケットに載っている送信時刻とカーネルが付けた受信時刻を比べる。
...
10 分のあいだに 598 個の秒が届いた。
どのパケットも `mode` が broadcast、`stratum` が 1、`refid` が `GPS`、`root dispersion` が 1.007 ms で、記録した 1069 行すべてで一致している。

行数がパケット数の倍近いのは、同じ broadcast がこのホストに二つの経路から入るからである。
その仕組みと見分け方は次節で扱う。
598 個はすべて有線で受けたもので、無線で受けた分はその部分集合になる。

有線で受けた 598 パケットについて、送信時刻から受信時刻を引くと、平均 **−3816.5 µs**、標準偏差 **177.4 µs** だった。

![有線側の分布](fig-wired.png)

この平均は 4 つの合計で、ここでは分けられない。
送信時刻の申告そのものの誤り、送出から線に出るまでの遅れ、線からカーネルが受信時刻を付けるまでの経路、そして受信ホストの時計の誤差である。
broadcast は受信側から経路を測れないので、この記録だけでは切り分けられない。
このうち firmware の申告と線に出るまでの遅れは、後の節でオシロを当てて測る。
受信経路とホストの時計は、さらに後の節で外部の基準に当てて追う。
...
どちらから来たのかはカーネルに聞く。
`IP_PKTINFO` を有効にすると、パケットごとに補助データ (ancillary data) で受信インタフェースの番号が返る。
...
その集まり具合を最大にする T は **307.1995 ms** で、そこから決まる勾配は 78.401 ms/s である。
...
## broadcast の client は経路を測れない

unicast の client は要求と応答の 4 つのタイムスタンプから経路の遅延を測り、その半分を引いて時刻を決める。
broadcast にはその往復がない。
RFC 5905 §3 は、broadcast client がまず client mode で数往復して伝搬遅延を校正し、そのあとで受信だけに移ることを想定している。
この配線では往復そのものが張れないので、その校正ができない。

つまり二つの経路の差は、そのまま client の時刻の差になる。
有線なら動かない部分が 3.8 ms、揺れが 0.18 ms である。
無線なら動かない部分が 136 ms、揺れが 90 ms である。
無線の 90 ms は校正で引ける量ではない。
...
firmware から見える範囲では、申告した時刻から、組み立てたフレームを DMA へ渡すまでが中央値 **118.4 µs** (n=389、102.4–133.4、標準偏差 5.4 µs) である。
...
掴んだのがフレームであってリンクパルスでないことは、信号が続いた長さで確かめた。
60 発すべてで 81.8 µs 続いていて、102 byte を 10 Mbit/s で送る計算値 81.6 µs と合う。
...
直し方は 2 つある。
その分だけ早く送り始めるか、申告する時刻を実際の送出に合わせるかである。

早く送り始めるほうを先に試して、悪化した。
firmware は秒境界の手前まで待ってからフレームを渡すのだが、その待ちがリンクパルスの 16 ms 間隔のどこで明けるかは決まっていない。
そこからさらに 118 µs 手前を狙うと、待つ時間が残っていない回が出る。
残差は 118.4 µs から 0 へは行かず 73.5 µs までしか下がらず、標準偏差は 5.4 µs から 26.5 µs へ広がり、13% の秒は待たずにそのまま渡していた。
...
RFC 5905 の precision は system clock のものと定義されていて、送信時刻を付けるときの誤差は厳密にはそれでも root dispersion でもないが、プロトコルに置き場がない。
...
[pps-nmea-pairing](../pps-nmea-pairing/README.md) が書いた盲点の一段下に、同じものがある。
