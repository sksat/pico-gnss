...
### 残っているのは、やりとりが分けられない量

平均 −124 ns が残る。これはループでは届かない。

両方の板が自分のパッドを自分の秒に置いており、client はそれを 1 ns ほどでやっている。
残るのは **client の秒が server の秒とどれだけ違うか**で、それを決めているのはやりとりである。

そして two-step の E2E は、**方向ごとに長さの違う経路と、時計のずれとを分けられない**。
経路遅延は 2 方向の和で、offset は差だからである。片方向にだけ乗る量は前者では打ち消し、
後者には丸ごと入る。実装の問題ではなく、この機構そのものの性質である。

手がかりはある。同じリンクをタイムスタンプで実測した値は **+592 ns** で
([フレームの時刻を PIO で記録する](../pico-link-ptp/))、やりとりが報告する経路遅延は **+4 ns**
である。588 ns 食い違うので、**タイムスタンプを打っている点はフレームの同じ位置ではない**。
それが 2 枚と 2 方向で同じなら経路遅延だけが狂って offset には出ないが、違えばその差が offset に
乗る。どちらであるかはまだ見ていない。

分けるには 2 枚の役割を入れ替えて同じ測定を取るのがいちばん近い。
