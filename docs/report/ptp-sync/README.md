...
分けるには **loopback** が要る。1 枚の板で GP16 と GP18 をジャンパし、同じフレームを出口と入口の
両方の state machine でタイムスタンプすれば、`t_arrival − t_egress` が配線ぶんで、残りが 1 枚の
板の中の固定オフセットとして 1 つの数字に切り出せる。
配線を組み替える必要があるので、この回では測っていない。
...
gap が**配る時刻**に効かないことは恒等式からの帰結だが、ピンの上では確かめていない。
gap を振りながら client の GP6 を測れば直接見えるはずで、この回はオシロが波形の転送を
受け付けなくなったところで止まっている。
