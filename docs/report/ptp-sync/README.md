...
| | 直す前 | 直したあと |
|---|---:|---:|
| server GP6 − 受信機の秒 | −69 / −79 / −84 / −102 ns | **+4.0 / −5.4 / +13.7 / +10.0 ns** (sd 15 〜 32) |
| client GP6 − server GP6 | +338 / +356 / +411 / +513 ns | **−16.8 / +3.1 / −7.0 / +13.4 ns** (sd 26 〜 61) |
| 報告される経路遅延 | −165.5 ns | **+4.0 ns** (sd 4.7) |

**offset も jitter も、4 run すべてで 2 桁 ns に入った。**
はじめは offset が +355.6 ns、jitter が 136.6 ns だった。
...
この run の offset は +13.4 ns、jitter は 33.4 ns、1 発ごとの値は −53 から +79 ns に散る。
**平均だけでなく、1 発ごとの値も 100 ns の内側にある。**

### 最後に残っていたのは、折り返しそのものだった

ここまでで、板が自分について確かめられることはすべて合っていた。

- server のパッドは受信機の秒の上にある。オシロで数 ns。
- client のパッドは自分の時計の秒の上にある。`late_ns` +25 ns に対し `pin_late_ns` −24 ns で、
  差し引き 1 ns ほど。
- やりとりが報告する経路遅延は +4 ns で、折り返しを 16 倍に振っても動かない。

それでも 2 枚のパッドは 124 ns 離れていた。
残っていたのは **client の秒と server の秒のあいだ**、つまり配る時刻そのものである。

入力経路の差ではない。
server のピンのループは GP6 と GP2 の**捕捉**を突き合わせて 0 にしており、その結果として
**パッド**が数 ns で揃っているとオシロが言う。入力経路が 124 ns 違えばパッドはそれだけ離れる。

**その量は折り返し時間に比例していた。**
`DELAY_REQ_PHASE_MS` を 200 から 20 に変えるだけで、124 ns が 17 ns になる。

| 折り返し | client GP6 − server GP6 |
|---:|---:|
| 179 ms | −109.7 / −154.3 / −108.7 ns |
| 22 ms | **−16.8 / +3.1 / −7.0 / +13.4 ns** |

**200 ms はこちらが置いた値だった。**
`Delay_Req` を自分の秒の決まった位相で出させると、その到着位相が相手の秒に対する時計の誤差になる —
という診断のために選んだもので、それ以外の根拠は無かった。
待っているあいだ slave は自分のカウンタの上にいるので、待つ時間に比例する量はすべて待つ時間に
比例して減る。
待つ理由があるのは相手の受信機のほうだけで、1 フレームを復号し終えるまでは次を送っても見えない
(`PTP_GAP_MS` が置かれているのと同じ制約)。20 ms はそこを越えている。
**リンクが許す最短にする**のが正しく、これは測定に合わせて選ぶ数ではない。

## 何が残っているか

**折り返しに比例する残りの係数が分かっていない。**
周波数項の補正は、報告される経路遅延からはこの量を取り切っている — だから path は折り返しに
依らない。offset からは取り切れていない。
残りも折り返しに比例するが、その係数は推定した `δ` では説明できない。
待つ時間を短くすると比例して小さくなるので、いまは 2 桁 ns の内側にあるが、**なぜ残るのかは
分かっていない**。
...
**two-step の E2E は、方向ごとに長さの違う経路と時計のずれとを分けられない。**
経路遅延は 2 方向の和で、offset は差だからである。片方向にだけ乗る量は前者で打ち消し、後者に
丸ごと入る。いまの経路遅延は +4 ns で静かなので、非対称が大きいという積極的な証拠は無い。
上の 2 つを分けるには、2 枚の役割を入れ替えて同じ測定を取るのがいちばん近い。
非対称は測定値の中で符号が変わり、板ごとの固定の遅れは変わらないので、2 つの測定で 2 つの未知数が
解ける。

**両端が PIO で打った値だけで確かめようともした。**
client は自分の `Delay_Req` が自分の秒のどこで出たかを、server は同じ frame が自分の秒のどこに
着いたかを、どちらもパッドの捕捉から出す。
中間 200 ms に対して両者は 1269 ns しか違わなかったが、この差にはそれぞれの板の
「schedule の縁とパッドのあいだ」の定数が入っているので、2 枚の秒の差だけを取り出せない。

**jitter は 2 桁 ns だが、run ごとに割れる。**
client − server の sd は同じ条件で 26 から 61 ns まで動く。
1 回の測定の分解能は 8 ns 格子で、雑音の床は σ = 4.62 ns だから、これは分解能ではない。
run ごとに割れることは server 単体でも起きていて (「run ごとに jitter が 1.6 ns と 51.6 ns に
割れる」)、そちらと同じものかどうかは確かめていない。
