...
client には基準にする 1PPS が無いが、**閉じる相手は物理のパルスでなくてよい** — PTP で規律した
秒でよい。PIO1 で出力の state machine と GP6 を見る state machine を同時に起動し、

```text
捕捉した GP6 のエッジ − PpsSchedule が予測した GP6 のエッジ
```

を測れば、それが「出力プログラム + パッド + 入力捕捉」の固定オフセットになる。
これを PTP から計算した遅れに足せば、client の GP6 も PTP の秒に対して閉ループになる。
起動時にソフトウェアの時計を 1 回読んでいる誤差も、同じ定数として一緒に落ちる。

いまの client − server は −1174 ns (sd 270 ns) で、その大半がこの開ループの定数である。
床は PTP 側 (経路の非対称、client の規律の残差、パケットのタイムスタンプの雑音) になる。
...
### 残る 350 ns は、どちらの firmware も自分では見えていない

同じ run で、server は自分のパルスが受信機の秒から **−16 ns** のところにあると報告し、client は
自分のパルスが自分の秒から **+6 ns** のところにあると報告する。
**両方とも「自分は合っている」と言っていて、ピンの上では 350 ns 離れている。**

差は PTP が渡す時刻そのものにある。
経路遅延は −38 ns で静かなので、**方向ごとの非対称**が offset に残っている勘定になる
(経路遅延はその和で、offset はその差なので、片方だけに乗る量は経路遅延では見えない)。

板が同じなら非対称は相殺する。相殺しないのは 2 枚の**時刻の作り方が違う**からで、server は
1PPS のアンカーに対する `ticks_between` から、client はカウンタのタイムラインから作っている。
どこで 350 ns 分かれるかは特定できていない。

累積ではない。1 つの run のなかで前半と後半を比べると ±110 ns 動いていて、向きは run ごとに
違う。捕捉回数の数え間違いのように一方向へ積もるものなら、こうはならない。
**固定のバイアスに、ゆっくりした揺れが乗っている**形である。

切り分けるには、1 回のやりとりの t1 t2 t3 t4 を個別に、**UTC の基準に対して**記録するしかない。
それぞれの差を秒で折り返してみたが、client の時刻には任意のエポックが乗っているので、両方の
leg が ±497 ms に折り返されて何も見えなかった。両端の firmware が自分では見えない量なので、
外から見るほかない。

**受信機と server のあいだは 2 桁 ns に入った。**
2 枚のあいだは 400 〜 500 ns で、2 run で再現している — µs のさまよいではなく固定のバイアスである。

残る 450 ns は PTP の offset に乗っている。経路遅延は −38 ns で静かなので、方向ごとの非対称
(offset には残り、経路遅延では打ち消し合う量) が 900 ns ある勘定になる。
その出どころはまだ分かっていない。
