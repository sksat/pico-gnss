...
| | 直す前 | 直したあと |
|---|---:|---:|
| server GP6 − 受信機の秒 | −69 / −79 / −84 / −102 ns | **+8.1 / +7.7 / +6.5 / +11.9 ns** (sd 23 〜 30) |
| client GP6 − server GP6 | +338 / +356 / +411 / +513 ns | **−109.7 / −154.3 / −108.7 ns** (sd 33 / 50 / 33) |
| 報告される経路遅延 | −165.5 ns | **+4.0 ns** (sd 4.7) |

**jitter は 3 run とも 2 桁 ns になった** (32.5 / 49.9 / 32.9)。
はじめは 136.6 ns だったので、ここは 3 倍以上縮んでいる。

**offset は 2 桁に入っていない。** 3 run の平均は −124 ns である。
ただし、これはもうループの話ではない (下の「残っているのは、やりとりが分けられない量」)。
...
ここまでの jitter は外来の雑音ではなく、**server 自身のループが鳴っている音**だった。

受信機は静かである。この板の水晶で測った 1PPS の間隔は 1e9 − 142.1 ns、sd 10.1 ns (n=34) で、
mean は水晶の周波数誤差、sd は捕捉 2 つの量子化 (σ 6.5 ns) とほぼ同じである。
それでも server 自身のピンは受信機の秒に対して ±150 ns 動いていた。

その動きの自己相関が、符号を交互に振りながら減衰する。

| run | n | sd | r1 | r2 | r3 | r4 |
|---|---:|---:|---:|---:|---:|---:|
| A | 92 | 71.1 | −0.82 | +0.58 | −0.41 | +0.30 |
| B | 27 | 111.9 | −0.88 | +0.75 | −0.64 | +0.48 |
| C | 31 | 30.8 | +0.20 | −0.22 | −0.47 | −0.43 |

ランダムウォークなら正のまま単調に減り、白色雑音なら r1 ≈ 0 になる。
交互符号は減衰振動であり、**振動が強い run ほど sd が大きい**。

構造を見ると理由がある。
出力の誤差は配る map を通して読まれ、その map を同じ誤差が動かしていた。
**同じ物理量を 2 つの補正器が、それぞれ 1 秒の遅れを持って駆動していた**。
しかもプラント (周期 → 位相) 自体が積分器なので、そこに積分器を足すと原点に極が 2 つになる。
状態 2 つ、ゲインどちらも 1/4 の固有値は 0.875 ± 0.217i で、周期は 25.9 サンプル。
ログは 16 秒おきなので隣り合う記録が 0.62 周期ぶん離れ、符号が交互になって見える。

出力をピンの誤差だけで操舵し、map をその経路から外した。
プラントが積分器なので比例で足り、状態がもう 1 つ増えないので鳴る相手がいない。
`rp-pps/tests/pin_loop.rs` に、2 つの形が**次数で違うのであってゲインの調整で違うのではない**こと
と、鳴りの周期が固有値の言うとおりであることを、プラントモデルとして置いた。
...
減衰の取れた積分器も試した。
臨界減衰は `Ki = Kp²/4` で、これは選ぶ数ではなく `Kp` から決まる関係である
(出荷時は `Ki = Kp` で、16 倍だった)。
鳴りは止まったが、落ち着き先は良くならなかった — 58 ns 対 21〜38 ns。
積分器が消すのは「周波数推定が外している量の 1/Kp 倍」の定常誤差 (ここでは 7 ppb の 4 倍で 28 ns)
で、その代わりに測定の雑音を周波数へ積む。この雑音では割に合わない。
どちらが得かを決めるのは受信機の雑音と水晶自身のさまよいが交わる場所で、それは基板の性質だから、
ゲインをそこから決めるなら**両方を実行時に推定する**ことになる。数を書き込むことではない。

### client のループは、自分の定常誤差を持っていた

client 側も同じ形に直した。
自分のピンを見てはいたが報告するだけで、置いていたのは schedule が思うエッジのほうだった。
パッドを置くように変えると、2 枚の差は −145.9 ns から −82.0 ns になった。

それでも client のループは定常位相誤差を持っていた。
同じ run で `pin_late_ns` は −80 ns で**完全に一定**なのに、`late_ns` は **+52 〜 +79 ns** に居座り、
run ごとに動く。比例ループは周波数誤差 `e` に対して `e / Kp` の位相を持たなければ補正を作れないので、
17 ppb ほど外している推定に対して 70 ns ほどが立つ。

ここでは積分器が割に合う。
server で割に合わなかったのは、そこで積むのが受信機の秒を捕捉 2 つ越しに見た値だからで、client が
積むのは既に時計へ均されたやりとりである。ゲインは同じ関係 (`Ki = Kp²/4`) で、比例側が飽和して
いるあいだは積まない。

入れると `late_ns` は **+25 ns、sd 5.0** に落ち着く。`pin_late_ns` が −24 ns なので、**パッド自身は
自分の秒の上に 1 ns ほどで乗っている**。

| client のループ | 3〜4 run の平均 | run 間の幅 | jitter |
|---|---:|---:|---:|
| 比例のみ | −124.1 ns | 128 ns | 20 〜 56 ns |
| 積分あり | −124.2 ns | **46 ns** | 33 〜 50 ns |

平均も jitter も変わらず、**run をまたぐさまよいが 3 分の 1 になった**。
さまよっていたのは、この定常誤差だったということである。
...
この run の offset は −128.3 ns、jitter は 45.3 ns、1 発ごとの値は −204 から −25 ns に散る。
**平均は 100 ns の桁で、1 発ごとの幅もその桁に収まっている。**
...
### 残っているのは、二つの秒のあいだの量である

平均 −124 ns が残る。これはループでは届かない。

**板が自分について確かめられることは、すべて合っている。**

- server のパッドは受信機の秒の上にある。オシロで +3 ns。
- client のパッドは自分の時計の秒の上にある。`late_ns` +25 ns に対し `pin_late_ns` −24 ns で、
  差し引き 1 ns ほど。
- やりとりが報告する経路遅延は +4 ns で、ジャンパ 2 本として妥当な正の値である。

それでも 2 枚のパッドは 124 ns 離れている。
残っているのは **client の秒と server の秒のあいだ**、つまり配る時刻そのものである。

入力経路の差ではない。
server のピンのループは GP6 と GP2 の**捕捉**を突き合わせて 0 にしており、その結果として
**パッド**が +3 ns で揃っているとオシロが言う。入力経路が 124 ns 違えばパッドはそれだけ離れるので、
受信機のピンとリンクのピンの入力経路は数 ns で一致している。

two-step の E2E にも限界がある。**方向ごとに長さの違う経路と、時計のずれとを分けられない** —
経路遅延は 2 方向の和で、offset は差だからである。片方向にだけ乗る量は前者で打ち消し、後者に丸ごと
入る。ただし経路遅延は +4 ns で静かなので、非対称が大きいという積極的な証拠も無い。

**両端が PIO で打った値だけで確かめようともした。** client は自分の `Delay_Req` が自分の秒の
どこで出たかを、server は同じ frame が自分の秒のどこに着いたかを、どちらもパッドの捕捉から出す。
中間 200 ms に対して両者は 1269 ns しか違わなかったが、この差にはそれぞれの板の
「schedule の縁とパッドのあいだ」の定数が入っているので、2 枚の秒の差だけを取り出せない。
同じ観測不能性である。

分けるには 2 枚の役割を入れ替えるのがいちばん近い。
非対称は測定値の中で符号が変わり、板ごとの固定の遅れは変わらないので、2 つの測定で 2 つの未知数が
解ける。
...
### 残っているのは、二つの入力経路の差である

平均 −124 ns が残る。これはループでは届かない。
両方の板が自分のパッドを自分の秒に置いており、client はそれを 1 ns ほどでやっているからである。

どこに居るかは追える。

server が配る時刻は「カウンタの値 → UTC」の写像を通る。
その写像は **受信機の 1PPS の捕捉** にアンカーされ、NMEA の秒と結ばれている。
一方、実際に時刻を打つ相手は **リンクのピンの捕捉** である。
両方とも同じ捕捉プログラムを走らせ、同じ toll を払い、同じブロックの同じ書き込みで起動している —
だからここに差があるとすれば、コードではなく**信号とパッド**の差である。
受信機の 1PPS は入力で反転して見ており、その立ち下がりはリンクの速い Manchester 遷移とは違う速さで
入力の閾値を跨ぐ。

その差を `D` とすると、写像は真の時刻から `D` ずれた秒を定義し、**それが配る時刻すべてに乗る**。
client はその秒に自分のパッドを置くので、2 枚の差にそのまま出る。

**どちらの板も自分では見えない。**
server のピンのループが比べているのは GP6 と GP2 で、これは別の対である。
その対のずれはループが 0 にしており、オシロもそう言う (+3 ns)。
問題の対 — 受信機のピンとリンクのピン — は、どちらのループにも入っていない。

測るには、**1 本の物理エッジを両方のピンに入れて、同じプログラムで捕らえる**のが直接である。
state machine はピンを所有しなくても番号で見られる (`set_jmp_pin_unclaimed`) ので、必要なのは
ジャンパ 1 本と、両方を同じブロックの空いた 2 つの machine に向けることだけである。
リンクの 2 本のカウンタを突き合わせたとき (下) と同じやり方が、そのまま使える。

やりとりの側にも限界があることは変わらない。
two-step の E2E は**方向ごとに長さの違う経路と、時計のずれとを分けられない** — 経路遅延は 2 方向の
和で、offset は差だからである。片方向にだけ乗る量は前者で打ち消し、後者に丸ごと入る。
いまの経路遅延は +4 ns で静かなので、リンク自身が非対称である証拠は無い。
それでも分けたければ、2 枚の役割を入れ替えて同じ測定を取るのがいちばん近い。
非対称は測定値の中で符号が変わり、板ごとの固定の遅れは変わらないので、2 つの測定で 2 つの未知数が
解ける。
