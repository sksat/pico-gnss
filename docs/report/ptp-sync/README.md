...
`δ` は独立な 2 通りの推定が一致している。
`PTP_GAP_MS` を振ったときの傾きが −0.840 ns/ms で 1.68 ppm、t2 − t1 のドリフトが +1730 ns/s で
1.73 ppm である。

`PTP_GAP_MS = 20` のとき、周波数項は −19.2 ns になる。
これが `d = +18.2 ns` をほぼ打ち消すので、報告値が 0 の近くに来る。
gap を 60 ms にすると報告値は −34.9 ns へ動く。
...
どちらの run でも報告値は 8 ns 間隔の 3 クラスタに乗っていて (注入ありでは
−45.5 / −37.5 / −29.5)、1 回の測定が格子より細かく読めているわけではない。
...
同じ配線、同じ server バイナリ、落ち着き 240 秒、90 取り込みで、NTP 駆動と PTP 駆動を比べた。
値は「offset ± jitter」である。

**この 2 つの run は、上のカウンタ初期化の修正より前の firmware で取っている。**
修正後に測り直そうとしたが、オシロが単発トリガを取れない状態になり、この回では取れていない。
差の桁 (70.6 µs → 2.2 µs) は残差 ±20 ns の 3 桁上なので結論は動かないが、修正後の値そのものは
まだ無い。

| client の時計 | client GP6 − 受信機の 1PPS | client GP6 − server GP6 |
|---|---:|---:|
| NTP 駆動 | +68026.8 ± 1747.3 ns | +70595.2 ± 1733.1 ns |
| PTP 駆動 | −351.1 ± 471.3 ns | **+2196.8 ± 337.1 ns** |

server 自身の GP6 は、受信機に対して NTP 駆動で −2568.4 ± 476.2 ns、PTP 駆動で
−2547.9 ± 468.6 ns だった。
同じバイナリなので、これは server が動いていないことの確認になる。

2 枚の板の 1PPS の差は 70.6 µs から 2.2 µs へ、揺れは 1733 ns から 337 ns へ縮んだ。

**−351.1 ns を PTP の転送誤差として読むことはできない。**
...
揺れのほうも、大半は PTP のものではない。
client の 471 ns と server の 469 ns はほぼ等しく、2 枚の差の揺れ (337 ns) はそのどちらよりも
小さい。
独立なら √(471² + 469²) = 665 ns になるはずなので、出力チェーンの揺れは 2 枚に共通で、そこが
支配的だと読める。
PTP がそこに足しているものは、この測定では見えない。

## 何が残っているか

**固定残差 +18.2 ns の正体が分かっていない。**
報告値の中央値そのものは run をまたぐと ±18 ns 動くが、その動きは gap の違う周波数項でほぼ
説明がつき、差し引いたあとの残差は 4 つの run で ±0.3 ns しか動かない。
1 回の測定がこの細かさで読めるという意味ではなく、多数標本の中央値どうしがこれだけ揃う、という
意味である。
ばらつきというより、計器の固定オフセットとして扱うのが自然な量になっている。
ジャンパ 2 本の伝搬時間だけでこの大きさになるとは考えにくいが、この測定は伝搬時間と 2 枚の板の
捕捉チェーンの差を分けられない。
TODO: 1 枚の板で GP16 と GP18 をジャンパし、同じフレームを出口と入口の両方の state machine で
タイムスタンプする (loopback)。
`t_arrival − t_egress` が配線ぶんで、残りが 1 枚の板の中の固定オフセットとして切り出せる。
...
TODO: `PTP_GAP_MS` を 2 ms にした run で報告値が +16 ns 付近へ動くか (モデルからの予測は +16.3 ns)。
TODO: gap を振ったときに client の GP6 がオシロ上で動かないことの確認。
配る時刻に効かないのは恒等式からの帰結だが、実測では確かめていない。

**出力チェーンが、やりとりより 30 倍から 60 倍粗い。**
exchange が分けられるのは 8 ns で、出力の揺れは 470 ns である。
...
TODO: 図と、その図を生データから描くスクリプトを `docs/report/ptp-sync/logs/` に置く。
本文の数値も同じスクリプトから出す。
