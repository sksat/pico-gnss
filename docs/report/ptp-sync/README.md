...
| client GP6 − server GP6 | +338 / +356 / +411 / +513 ns | **−82.0 / −121.7 / −82.6 / −209.9 ns** (sd 20 〜 56) |
| 報告される経路遅延 | −165.5 ns | **+4.0 ns** (sd 4.7) |

**jitter は 4 run とも 2 桁 ns になった** (20.3 / 56.4 / 30.4 / 44.7)。
はじめは 136.6 ns だったので、ここは 4 倍以上縮んでいる。

**offset は 2 桁に安定していない。**
4 run が −82.0 から −209.9 まで 128 ns の幅で動く。
2 桁に入っているのは 4 つのうち 2 つである。
...
### 残る offset は、比例ループの定常誤差である

client 側も同じ形に直した。
自分のピンを見てはいたが報告するだけで、置いていたのは schedule が思うエッジのほうだった。
パッドを置くように変えると、2 枚の差は −145.9 ns から −82.0 ns になった。

残りは client のループが持っている定常位相誤差である。
同じ run で `pin_late_ns` は −80 ns で**完全に一定**なのに、`late_ns` は **+52 〜 +79 ns** に居座る。
比例ループは周波数誤差 `e` に対して `e / Kp` の位相を持たなければ補正を作れないので、17 ppb ほど
外している推定に対して 70 ns ほどの定常誤差が立つ。符号も、client のパッドが早いことと合う。

これを消すのは client 側の積分器だが、server で割に合わなかったのと同じ取引になる。
違うのは client の基準のほうがいまは静か (受信機の秒に対して sd 11 〜 37 ns) なことで、
同じ取引が同じ結論になるとは限らない。試していない。
...
**offset が run をまたいで 128 ns 動く。**
4 run が −82.0 / −121.7 / −82.6 / −209.9 ns である。
そのうち 70 ns ほどは client の比例ループの定常誤差として説明がつくが、run ごとに動く量は
説明できていない。周波数推定が run ごとに違うところに落ち着くのは見えているので、そこが
候補である。
